 ...
 * @Description: mc-central-ts: save-docFile-record(s) (create/insert and update record(s))
 */
...
import { deleteHashCache } from "../../../mc-cache";
...
class SaveDoc extends Crud {
    ...
    async saveDoc(): Promise<ResponseMessage> {
        ...
        if (this.createItems && this.existParams && this.createItems.length > 0 && this.createItems.length <= this.existParams.length) {
            try {
                ...
                const taskPermitted = await this.taskPermission(TaskTypes.CREATE);
                if (taskPermitted.value.ok && taskPermitted.code !== 'success') {
        ...
        if (this.existParams && this.updateItems && this.updateItems.length > 0 && this.updateItems.length <= this.existParams.length) {
            try {
                ...
                // check/validate action permissions
                const taskPermitted = await this.taskPermission(TaskTypes.UPDATE);
                if (taskPermitted.value.ok && taskPermitted.code === "success") {
                    return taskPermitted;
                }
        ...
        if (this.isAdmin && this.docIds && this.docIds.length < 1 && this.queryParams && Object.keys(this.queryParams).length > 0 && this.actionParams && this.actionParams.length === 1) {
            try {
                // check duplicate records, i.e. if similar records exist
                const recExist = await this.checkRecExist();
                if (recExist.code !== 'success') {
...
function newSaveDoc(params: CrudTaskType,
                    options: CrudOptionsType = {}) {
    return new SaveDoc(params, options);
}

export { SaveDoc, newSaveDoc };
